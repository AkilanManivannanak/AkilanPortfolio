<template>
  <div id="app" class="flex">
    <NetflixBackground v-if="loggedIn" />
    <ToastContainer />
    <AlertsContainer />
    ...
    <PortalTarget multiple style="z-index: 10500" class="fixed top-0 left-0" name="Preview-Outlet" />
...
export default class App extends Vue {
  get loggedIn() { return AuthModule.state.loggedIn; }

  public showPreview = false;
  public projectId = '';
  public experienceId = '';

  get routeParam() { return this.$route.query; }

  @Watch('routeParam', { deep: true, immediate: true }) routeChanged() {
    const { jbv, xp } = this.$route.query;
    if (jbv && typeof jbv === 'string') {
      this.showPreview = true; this.projectId = jbv; this.experienceId = '';
    } else if (xp && typeof xp === 'string') {
      this.showPreview = true; this.experienceId = xp; this.projectId = '';
    } else {
      this.showPreview = false; this.projectId = '';
    }
  }
  ...
  created() {
    if (process.browser) {
      ...
      if (auto) DarkModeModule.actions.enableAutoDetection();
      else if (stored) DarkModeModule.actions.setTheme(stored as DisplayTheme);
      else DarkModeModule.actions.setTheme('dark');
    }
    if (process.browser) {
...
<style lang="postcss">
div#app {
  position: relative; width: 100%; flex: 1; max-width: 100%;
  background: #141414;
  #rootView {
    position: relative; width: 100%; max-width: 100%; flex: 1;
  }
}
</style>
